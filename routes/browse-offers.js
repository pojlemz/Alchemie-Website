...
// Get Homepage
router.get('/browse-offers', ensureAuthenticated, function(req, res){
    var response = res;
    HasBeenKyced.getHasBeenKycedByEmail( req.user.email, function(err, res) {
        if (res !== null && res.kyced) {
            response.render('browse-offers'); // in case where kyc is completed but user finds this link anyways
        } else {
